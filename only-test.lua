...
-- [ GLOBAL CFG ]
_G.TP_METHOD = "classic"
_G.CENTRAL_LOADED = true
_G.EXE = { FARM_RUNNING = false, FARM_THREAD = nil, BYPASS_CARS_ON = false }

-- [ BYPASS TP ]
local TP_ENV = { METHOD = "classic", SP_PT = nil, SP_PS = nil }
...
local function BYPASS_TP(targetPos)
    ...
    if _G.TP_METHOD == "classic" then
        TP_CLASSIC(targetPos)
    elseif _G.TP_METHOD == "scooter" then
...
local function BUILD_NYH_UI()
    ...
    ; (function()   -- register isolation: MISC right col
        ...
        for i, gun in ipairs(GUNS) do
            ...
            ITM.MouseButton1Click:Connect(function()
                ...
                if ok then
                    NOTIFY("Guns", "✓ " .. gun.name .. " bought!", 3)
                else
                    NOTIFY("Guns", "Error: " .. tostring(err), 4)
                end
            end)
        end
        ...
        for i, mod in ipairs(MODS) do
            ...
            ITM.MouseButton1Click:Connect(function()
                ...
                if not targetTool then
                    NOTIFY("Gun Mods", "❌ Equip the gun first!", 4)
                    return
                end
                ...
                local ok, err = pcall(function()
                    ...
                    -- SECURE RE-EQUIP (Specific Instance)
                    if targetTool and targetTool.Parent == LPLR.Backpack then
                        hum:EquipTool(targetTool)
                        task.wait(0.3)
                    end

                    -- Buy mod using the SPECIFIC tool
                    game:GetService("ReplicatedStorage").Remotes.PurchaseMod:FireServer(
                        targetTool, mod.key, mod.type
                    )
                    task.wait(0.5)
                ...
                if ok then
                    NOTIFY("Gun Mods", mod.name .. " applied to " .. targetTool.Name, 3)
                else
        ...
        local function WM_MK_SLIDER(label, minV, maxV, defV, onChange, order)
            ...
            game:GetService("RunService").Heartbeat:Connect(function()
                ...
                local rx = math.clamp(
                    (game:GetService("UserInputService"):GetMouseLocation().X - TRACK.AbsolutePosition.X) /
    ...
    ; (function() -- register isolation: actions card
        ...
        -- ── LEVEL UP ────────────────────────────────────────
        MK_ACT_BTN("Level Up", 6, function()
            task.spawn(function()
                local REP = game:GetService("ReplicatedStorage")
                local playerData = LPLR:WaitForChild("Player_Data", 5)
                if not playerData then return end

                local bankVal = playerData:WaitForChild("Bank")
                local levelVal = playerData:WaitForChild("Level")
                local Vehicle_Purchase = REP.Remotes.Vehicle_Purchase

                if tonumber(bankVal.Value) < 100000 then
                    NOTIFY("Level Up", "⚠️ Need at least $100,000 in Bank!", 5)
                    return
                end

                local function clearNotifs()
                    pcall(function()
                        local target = LPLR.PlayerGui:FindFirstChild("notificationGui")
                        if target and target:FindFirstChild("Frame") then
                            for _, obj in pairs(target.Frame:GetChildren()) do
                                if obj.Name == "Notif" or obj:FindFirstChild("statusLabel") then
                                    obj:Destroy()
                                end
                            end
                        end
                ...
                for i = 1, 8 do
                    ...
                    line.Size = UDim2.new(1 / 8, 0, 1, 0)
                    line.Position = UDim2.new((i - 1) / 8, 0, 0, 0)
                    line.BackgroundColor3 = Color3.fromRGB(20, 40, 80)
                    line.BackgroundTransparency = 0.92
                    line.BorderSizePixel = 0
                end
                for i = 1, 5 do
                    local line = Instance.new("Frame", grid)
                    line.Size = UDim2.new(1, 0, 1 / 5, 0)
                    line.Position = UDim2.new(0, 0, (i - 1) / 5, 0)
                    line.BackgroundColor3 = Color3.fromRGB(20, 40, 80)
                    line.BackgroundTransparency = 0.92
                    line.BorderSizePixel = 0
                end

                local glowTop = Instance.new("Frame", scroll)
                glowTop.Size = UDim2.new(0.6, 0, 0, 3)
                glowTop.Position = UDim2.new(0.2, 0, 0, 0)
                glowTop.BackgroundColor3 = Color3.fromRGB(0, 120, 255)
                glowTop.BorderSizePixel = 0
                local glowGrad = Instance.new("UIGradient", glowTop)
                ...
                headerBar.BackgroundColor3 = Color3.fromRGB(0, 30, 70)
                ...
                headerFix.BackgroundColor3 = Color3.fromRGB(0, 30, 70)
                headerFix.BackgroundTransparency = 0.3
                headerFix.BorderSizePixel = 0

                local dot = Instance.new("Frame", headerBar)
                ...
                div.Position = UDim2.new(0.04, 0, 0, 44)
                div.BackgroundColor3 = Color3.fromRGB(0, 80, 180)
                div.BackgroundTransparency = 0.6
                div.BorderSizePixel = 0

                local bankIcon = Instance.new("TextLabel", card)
                bankIcon.Size = UDim2.new(0, 30, 0, 30)
                bankIcon.Position = UDim2.new(0, 14, 0, 56)
                bankIcon.BackgroundTransparency = 1
                bankIcon.Text = "$"
                bankIcon.TextColor3 = Color3.fromRGB(0, 200, 100)
                bankIcon.Font = Enum.Font.GothamBold
                bankIcon.TextSize = 18

                local bankLabel = Instance.new("TextLabel", card)
                bankLabel.Size = UDim2.new(1, -50, 0, 30)
                bankLabel.Position = UDim2.new(0, 44, 0, 56)
                bankLabel.BackgroundTransparency = 1
                bankLabel.Text = "$" .. tostring(bankVal.Value)
                bankLabel.TextColor3 = Color3.fromRGB(100, 255, 120)
                bankLabel.Font = Enum.Font.GothamBold
                bankLabel.TextSize = 17
                bankLabel.TextXAlignment = Enum.TextXAlignment.Left

                local bankSub = Instance.new("TextLabel", card)
                bankSub.Size = UDim2.new(1, -50, 0, 18)
                bankSub.Position = UDim2.new(0, 44, 0, 84)
                bankSub.BackgroundTransparency = 1
                bankSub.Text = "BANK BALANCE"
                bankSub.TextColor3 = Color3.fromRGB(60, 100, 160)
                bankSub.Font = Enum.Font.GothamBold
                bankSub.TextSize = 10
                bankSub.TextXAlignment = Enum.TextXAlignment.Left

                local div2 = Instance.new("Frame", card)
                div2.Size = UDim2.new(0.92, 0, 0, 1)
                div2.Position = UDim2.new(0.04, 0, 0, 108)
                div2.BackgroundColor3 = Color3.fromRGB(0, 80, 180)
                div2.BackgroundTransparency = 0.7
                div2.BorderSizePixel = 0

                local levelIcon = Instance.new("TextLabel", card)
                levelIcon.Size = UDim2.new(0, 30, 0, 30)
                levelIcon.Position = UDim2.new(0, 14, 0, 120)
                levelIcon.BackgroundTransparency = 1
                levelIcon.Text = "⭐"
                levelIcon.TextColor3 = Color3.fromRGB(255, 200, 50)
                ...
                levelLabel.Position = UDim2.new(0, 44, 0, 120)
                levelLabel.BackgroundTransparency = 1
                levelLabel.Text = "LVL " .. tostring(levelVal.Value)
                levelLabel.TextColor3 = Color3.fromRGB(100, 200, 255)
                levelLabel.Font = Enum.Font.GothamBold
                levelLabel.TextSize = 17
                levelLabel.TextXAlignment = Enum.TextXAlignment.Left

                local levelChange = Instance.new("TextLabel", card)
                levelChange.Size = UDim2.new(0.5, -10, 0, 30)
                levelChange.Position = UDim2.new(0.5, 0, 0, 120)
                levelChange.BackgroundTransparency = 1
                levelChange.Text = ""
                levelChange.TextColor3 = Color3.fromRGB(100, 255, 150)
                levelChange.Font = Enum.Font.GothamBold
                levelChange.TextSize = 13
                levelChange.TextXAlignment = Enum.TextXAlignment.Right

                local levelSub = Instance.new("TextLabel", card)
                levelSub.Size = UDim2.new(1, -50, 0, 18)
                levelSub.Position = UDim2.new(0, 44, 0, 148)
                levelSub.BackgroundTransparency = 1
                levelSub.Text = "CURRENT LEVEL"
                levelSub.TextColor3 = Color3.fromRGB(60, 100, 160)
                levelSub.Font = Enum.Font.GothamBold
                levelSub.TextSize = 10
                levelSub.TextXAlignment = Enum.TextXAlignment.Left

                local div3 = Instance.new("Frame", card)
                div3.Size = UDim2.new(0.92, 0, 0, 1)
                div3.Position = UDim2.new(0.04, 0, 0, 172)
                div3.BackgroundColor3 = Color3.fromRGB(0, 80, 180)
                div3.BackgroundTransparency = 0.7
                div3.BorderSizePixel = 0

                local cyclesLabel = Instance.new("TextLabel", card)
                cyclesLabel.Size = UDim2.new(1, -20, 0, 28)
                cyclesLabel.Position = UDim2.new(0, 14, 0, 178)
                cyclesLabel.BackgroundTransparency = 1
                cyclesLabel.Text = "CYCLES  —  0"
                cyclesLabel.TextColor3 = Color3.fromRGB(80, 120, 180)
                cyclesLabel.Font = Enum.Font.GothamBold
                cyclesLabel.TextSize = 11
                cyclesLabel.TextXAlignment = Enum.TextXAlignment.Left

                local stopBtn = Instance.new("TextButton", card)
                stopBtn.Size = UDim2.new(0.88, 0, 0, 42)
                stopBtn.Position = UDim2.new(0.06, 0, 0, 215)
                stopBtn.BackgroundColor3 = Color3.fromRGB(0, 160, 60)
                stopBtn.BackgroundTransparency = 0.15
                stopBtn.TextColor3 = Color3.fromRGB(220, 255, 230)
                stopBtn.Text = "⏹   STOP LEVEL FARM"
                stopBtn.Font = Enum.Font.GothamBold
                stopBtn.TextSize = 13
                RND(stopBtn, 10)
                local btnStr = STR(stopBtn, Color3.fromRGB(0, 220, 80), 1)
                ...
                local function shutdown()
                    ...
                    stopping = true
                    running = false
                    stopBtn.Text = "⏹   STOPPING... (4s)"
                    TWN(stopBtn, { BackgroundColor3 = Color3.fromRGB(180, 30, 30) })
                    TWN(btnStr, { Color = Color3.fromRGB(220, 40, 40) })

                    -- User requested 4 second delay before closing
                    task.wait(4)

                    -- Fast Fade Optimization
                    local fi = TweenInfo.new(0.5, Enum.EasingStyle.Quart, Enum.EasingDirection.Out)
                    local fade = { bg, scroll, card, headerBar, title, bankLabel, levelLabel, cyclesLabel, stopBtn, logo,
                        glowTop,
                        dot, bankSub, levelSub, levelChange, bankIcon, levelIcon, headerFix, div, div2, div3 }
                    for _, o in pairs(fade) do
                        pcall(function()
                            if o:IsA("GuiObject") then TWN(o, { BackgroundTransparency = 1 }, 0.5) end
                            if o:IsA("TextLabel") or o:IsA("TextButton") then TWN(o, { TextTransparency = 1 }, 0.5) end
                            if o:IsA("ImageLabel") then TWN(o, { ImageTransparency = 1 }, 0.5) end
                        end)
                    end
                    TWN(cardStroke, { Transparency = 1 }, 0.5)
                    TWN(btnStr, { Transparency = 1 }, 0.5)
                ...
                stopBtn.MouseButton1Click:Connect(shutdown)

                -- Intro
                task.spawn(function()
                    TWN(logo, { ImageTransparency = 0 }, 0.6)
                    task.wait(0.3)
                    TWN(card, { BackgroundTransparency = 0.1 }, 0.5)
                    TWN(cardStroke, { Transparency = 0.6 }, 0.5)
                end)
                ...
                task.spawn(function()
                    while running do
                        ciclo = ciclo + 1
                        cyclesLabel.Text = "CYCLES  —  " .. ciclo
                        Vehicle_Purchase:InvokeServer("Ford LTD LX")
                        task.wait(1.1)
                        Vehicle_Purchase:InvokeServer("Ford LTD LX", "Sell")
                        task.wait(1.1)
                        if bankVal.Value < 40000 then
                            shutdown()
                            break
                        end
                    end
                ...
                -- Listeners
                local lastLvl = levelVal.Value
                local c1 = bankVal:GetPropertyChangedSignal("Value"):Connect(function()
                    bankLabel.Text = "$" .. tostring(bankVal.Value)
                end)
                local c2 = levelVal:GetPropertyChangedSignal("Value"):Connect(function()
                    local diff = levelVal.Value - lastLvl
                    levelLabel.Text = "LVL " .. tostring(levelVal.Value)
                    if diff > 0 then
                        levelChange.Text = "▲ +" .. diff
                        TWN(levelLabel, { TextColor3 = Color3.fromRGB(255, 240, 80) })
                        task.delay(0.5, function() TWN(levelLabel, { TextColor3 = Color3.fromRGB(100, 200, 255) }) end)
                    end
                    lastLvl = levelVal.Value
                end)
    ...
    local POTS_PRIORITARIOS = {
        { idx = 11, pos = Vector3.new(-119.85, 3.34, 822.89) },
        { idx = 12, pos = Vector3.new(-125.41, 3.34, 822.89) },
        { idx = 13, pos = Vector3.new(-131.15, 3.34, 806.17) },
        { idx = 14, pos = Vector3.new(-119.76, 3.34, 826.04) },
    }

    local TODOS_POTS = {
        { idx = 1,  pos = Vector3.new(-70.69, -6.70, -277.38) },
        { idx = 2,  pos = Vector3.new(-70.95, -6.70, -270.82) },
        { idx = 3,  pos = Vector3.new(-71.19, -6.70, -264.44) },
        { idx = 4,  pos = Vector3.new(-76.02, -6.70, -285.11) },
        { idx = 5,  pos = Vector3.new(-85.95, -6.74, -268.81) },
        { idx = 6,  pos = Vector3.new(-307.08, 17.21, 326.61) },
        { idx = 7,  pos = Vector3.new(-311.96, 17.21, 326.61) },
        { idx = 8,  pos = Vector3.new(-317.88, 17.21, 326.61) },
        { idx = 9,  pos = Vector3.new(-311.04, 17.29, 344.20) },
        { idx = 10, pos = Vector3.new(-306.19, 17.21, 340.45) },
        { idx = 11, pos = Vector3.new(-119.85, 3.34, 822.89) },
        { idx = 12, pos = Vector3.new(-125.41, 3.34, 822.89) },
        { idx = 13, pos = Vector3.new(-131.15, 3.34, 806.17) },
        { idx = 14, pos = Vector3.new(-119.76, 3.34, 826.04) },
        { idx = 15, pos = Vector3.new(-146.19, 18.82, 822.42) },
        { idx = 16, pos = Vector3.new(-153.30, 18.56, 805.12) },
        { idx = 17, pos = Vector3.new(-159.30, 18.56, 805.12) },
        { idx = 18, pos = Vector3.new(-165.29, 18.56, 805.12) },
        { idx = 19, pos = Vector3.new(-1297.87, 3.60, 1362.94) },
        { idx = 20, pos = Vector3.new(-1297.87, 3.60, 1356.10) },
        { idx = 21, pos = Vector3.new(-1297.78, 3.50, 1371.44) },
        { idx = 22, pos = Vector3.new(-1297.81, 3.60, 1399.39) },
        { idx = 23, pos = Vector3.new(-1323.42, 3.60, 1356.77) },
        { idx = 24, pos = Vector3.new(-1323.42, 3.60, 1365.00) },
        { idx = 25, pos = Vector3.new(-1323.42, 3.60, 1371.86) },
        { idx = 26, pos = Vector3.new(-1338.07, 3.50, 1374.05) },
        { idx = 27, pos = Vector3.new(-1338.07, 3.50, 1385.34) },
        { idx = 28, pos = Vector3.new(-1323.46, 3.60, 1398.01) },
    }
    ...
    local function F_WAIT_OFF(prompt, timeout)
        local t = 0
        while prompt.Enabled and t < (timeout or 30) do
            task.wait(0.1)
            t = t + 0.1
        end
    end
    ...
    local function F_WAIT_ON(prompt, timeout)
        local t = 0
        while not prompt.Enabled and t < (timeout or 120) do
            task.wait(0.1)
            t = t + 0.1
        end
    end
    ...
    local function F_GO_POT(data)
        ...
        -- Move closer to pot (approx 2 studs distance) for better interaction reliability
        F_TP(char, data.pos + Vector3.new(0, 3, 1.2))
        return char
    end
    ...
    local function F_HOLD(prompt)
        ...
        -- Anti-Cheat bypass: Force character to face the prompt/pot
        local char = LPLR.Character or LPLR.CharacterAdded:Wait()
        local hrp  = char:FindFirstChild("HumanoidRootPart")
        if hrp and prompt.Parent then
            local targetPos = (prompt.Parent:IsA("BasePart") and prompt.Parent.Position)
                or (prompt.Parent:IsA("Attachment") and prompt.Parent.WorldPosition)
            if targetPos then
    ...
    local function F_PLANT_WAVE_A(pots)
        ...
        for _, data in ipairs(pots) do
            ...
            if prompt then
                if not F_GET_POT(data):GetAttribute("Water") and F_COUNT("Water") > 0 then
                    char = F_GO_POT(data)
                    F_EQUIP(char, "Water")
                    F_HOLD(prompt)
                    F_WAIT_OFF(prompt, 10)
                    F_WAIT_ON(prompt, 10)
                    task.wait(0.2)
                end
            end
    ...
    local function F_BUY_WAVE()
        ...
        Purchase_R:FireServer("Watering Can", workspace.NPCs.Mdot)
        task.wait(1)
        for i = 1, 4 do
            Purchase_R:FireServer("Fertilizer", workspace.NPCs.Mdot)
            task.wait(1)
        end
        for i = 1, 4 do
            PurchaseSeeds_R:FireServer(FARM_PLANTA)
            task.wait(1)
        end
        task.wait(0.5)
        F_CLEAN()
    ...
    local function RUN_FARM_CYCLE()
        ...
        if FARM_CONFIG == 2 then
            ...
            for i = 1, #ALL_PLANTED, 4 do
                ...
                if ok2 and res2 then
                    NOTIFY("Farm Beta", "✅ Batch " .. math.ceil(i / 4) .. " delivered!", 4)
                else
        ...
        local got = F_BUY_WAVE()
        if not got then
            NOTIFY("Farm", "No items, stopping.", 4)
            return false
        end
        ...
        if #pots == 0 then
            NOTIFY("Farm", "No pots available!", 4)
            return false
        end
    ...
    ; (function() -- register isolation: plant shovel card
        ...
        ACT_BTN.MouseButton1Click:Connect(function()
            ...
            if ok and res then
                ...
                if diff > 0 then
                    NOTIFY("Bicarbonate", "✅ Purchased! Fertilizer increased +" .. diff .. " (Total: " .. lvlAfter .. ")",
                        5)
                else
    ...
    local function CC_DO_CYCLE()
        ...
        local n = CC_N_HORNOS
        local bicarb_ok = (LPLR:FindFirstChild("Job_Data") and LPLR.Job_Data:FindFirstChild("Bicarb") and LPLR.Job_Data.Bicarb.Value >= 1)
        local season_ok = (LPLR:FindFirstChild("Job_Data") and LPLR.Job_Data:FindFirstChild("Seasoning") and LPLR.Job_Data.Seasoning.Value >= 1)

        local needs_restock = CC_COUNT("Raw Gelatin") < n
        if bicarb_ok and CC_COUNT("Bi-Carb") < n then needs_restock = true end
        if season_ok and CC_COUNT("Seasoning") < n then needs_restock = true end
        ...
        if needs_restock then
            ...
            for i = 1, n do
                if CC_STOP_CHECK() then return end
                if bicarb_ok then
                    RS_CC.Remotes.Purchase:FireServer("Bi-Carb", workspace.NPCs.Paulie)
                    task.wait(0.7)
                end
                RS_CC.Remotes.Purchase:FireServer("Raw Gelatin", workspace.NPCs.Paulie)
                task.wait(0.7)
                if season_ok then
                    RS_CC.Remotes.Purchase:FireServer("Seasoning", workspace.NPCs.Paulie)
                    task.wait(0.7)
                end
            end
        ...
        for _, idx in ipairs(ovens) do
            ...
            for attempt = 1, 4 do
                ...
                -- Not received — retry immediately
                NOTIFY("Candy Farm", "Not received, retrying... (" .. attempt .. "/4)", 2)
            end
    ...
    ; (function() -- register isolation: candy farm UI
        ...
        -- ── UPGRADE SEASONING / BI-CARB BUTTONS ──────────────────
        local CC_InquireCooking = game:GetService("ReplicatedStorage").Remotes.inquireCooking

        local function MK_CC_UPGRADE_BTN(parent, txt, lo, cb)
            ...
            local BTN_STR = STR(BTN, CFG.COL.ACC, 1.2)
            BTN_STR.Transparency = 0.8
            BTN_STR.ApplyStrokeMode = Enum.ApplyStrokeMode.Border

            local BTN_GRAD = Instance.new("UIGradient", BTN)
            BTN_GRAD.Color = ColorSequence.new({
                ColorSequenceKeypoint.new(0, Color3.new(1, 1, 1)),
                ColorSequenceKeypoint.new(1, Color3.new(0.8, 0.8, 0.8))
            })
        ...
        -- Separador visual
        local CC_UPG_DIV = Instance.new("Frame", CC_CARD)
        CC_UPG_DIV.Size = UDim2.new(1, 0, 0, 1)
        CC_UPG_DIV.BackgroundColor3 = CFG.COL.ACC
        CC_UPG_DIV.BackgroundTransparency = 0.85
        CC_UPG_DIV.BorderSizePixel = 0
        CC_UPG_DIV.LayoutOrder = 4

        MK_CC_UPGRADE_BTN(CC_CARD, "⬆ Upgrade Seasoning", 5, function()
            local ok, res = pcall(function()
                return CC_InquireCooking:InvokeServer("Purchase", "Seasoning")
            end)
            if ok and res then
                NOTIFY("Candy Farm", "✅ Seasoning upgraded!", 4)
            else
                NOTIFY("Candy Farm", "❌ Seasoning upgrade failed.", 4)
            end
        end)

        MK_CC_UPGRADE_BTN(CC_CARD, "⬆ Upgrade Bi-Carb", 6, function()
            local ok, res = pcall(function()
                return CC_InquireCooking:InvokeServer("Purchase", "Bicarb")
            end)
            if ok and res then
                NOTIFY("Candy Farm", "✅ Bi-Carb upgraded!", 4)
            else
                NOTIFY("Candy Farm", "❌ Bi-Carb upgrade failed.", 4)
            end
        end)
    end)()
    ...
    do
        ...
        local function CF_DO_SKIM(atm, prompt)
            ...
            for tp_attempt = 1, 3 do
                ...
                cam.CameraSubject = char:FindFirstChildOfClass("Humanoid") or
                    LPLR.Character:FindFirstChildOfClass("Humanoid")
        ...
        CF_START_LOOP = function(T2_PILL, T2_KNOB)
    ...
    do  -- Card Farm UI block (split to avoid local register limit)
        ...
        local function SET_CF_TOG(ON)
            if ON then
                ...
                -- Stop immediately
                _G.EXE.CF_RUNNING = false
                if _G.EXE.CF_THREAD then
                    task.cancel(_G.EXE.CF_THREAD)
                    _G.EXE.CF_THREAD = nil
                end
                TWN(T2_PILL, { BackgroundColor3 = CFG.COL.GRY })
                TWN(T2_KNOB, { Position = UDim2.new(0, 3, 0.5, -9) })
    ...
    ; (function() -- register isolation: burger farm logic
        ...
        local function BF_INIT_SPAWN()
            ...
            if not assets then
                warn("[BF] spawn_Assets no encontrado")
                return
            end
            local points = assets:WaitForChild("Points", 10)
            if not points then
                warn("[BF] Points no encontrado")
                return
            end
        ...
        -- ─── CAMERA FOCUS ──────────────────────────────────────────
        local function BF_FOCUS_CAM(target)
            if not target then return end
            local pos = target
            if typeof(target) == "Instance" then
                if target:IsA("ProximityPrompt") then
                    local p = target.Parent
                    pos = (p:IsA("BasePart") and p.Position) or (p:IsA("Attachment") and p.WorldPosition) or p.Position
            ...
            if hrp then
                ...
                local diff = pos - headPos
                local dist = diff.Magnitude
                -- Keep camera 1.5 studs away from target to prevent overshooting or clipping
                local zoomDist = math.max(0, dist - 1.5)
                cam.CFrame = CFrame.new(headPos + diff.Unit * zoomDist, pos)
            end
        end
        ...
        local function BF_DO_CYCLE()
            ...
            -- 4. Check if we already have a Patty (equipped or in backpack)
            local hasPatty = BF_HAS("Patty")

            -- 5. TP a la nevera y obtener Patty (solo si no tenemos una)
            if not hasPatty then
                SHORT_TP(POS_FRIDGE)
                task.wait(0.4)

                local fridgePrompt = workspace.Jobs.Jerk_Center.ingredientsProx.ProximityPrompt
                BF_FOCUS_CAM(fridgePrompt)
                BF_FIRE(fridgePrompt)
                task.wait(1)
                BF_RESTORE_CAM()

                -- Verify we have Patty now
                if not BF_HAS("Patty") then
                    warn("[BF] Did not get Patty from fridge. Aborting cycle.")
                    return false
                end
            else
                -- Si ya tenemos la Patty, nos saltamos la nevera y procedemos
                -- NOTIFY("Burger Farm", "Ya tienes una Patty, procediendo al horno...", 2)
            end
    ...
    -- ── MOP FARM LOGIC (Janitor Deli) ──────────────────────────────────────────
    ; (function()
        local MF_RS       = game:GetService("ReplicatedStorage")
        local MF_SPAWN_PT = workspace.spawn_Assets.Points["Trinity Ave. Plaza"]
        local MF_CAM      = workspace.CurrentCamera

        local function MF_CAM_ZOOM(part)
            MF_CAM.CameraType = Enum.CameraType.Scriptable
            MF_CAM.CFrame = CFrame.new(part.Position + Vector3.new(0, 2, 4), part.Position)
        end

        local function MF_CAM_RESET()
            MF_CAM.CameraType = Enum.CameraType.Custom
        end

        local function MF_DO_TP(pos)
            local char   = LPLR.Character or LPLR.CharacterAdded:Wait()
            local hrp    = char:WaitForChild("HumanoidRootPart")
            hrp.Anchored = true
            MF_RS.Remotes.Spawn:FireServer(MF_SPAWN_PT)
            task.wait(0.3)
            hrp.CFrame   = CFrame.new(pos)
            hrp.Anchored = false
        end

        local function MF_IS_IN_JOB()
            local jd = LPLR:FindFirstChild("Job_Data")
            if not jd then return false end
            local cj = jd:FindFirstChild("CurrentJob")
            return cj and cj.Value == "Janitor"
        end

        local function MF_EQUIP_MOP()
            local char = LPLR.Character
            if not char then return end
            local hum = char:FindFirstChildOfClass("Humanoid")
            local mop = LPLR.Backpack:FindFirstChild("Mop") or char:FindFirstChild("Mop")
            if mop and hum then
                hum:EquipTool(mop)
                task.wait(0.3)
            end
        ...
        local function MF_START_JOB()
            if MF_IS_IN_JOB() then return end
            NOTIFY("Mop Farm", "Starting Janitor job...", 3)
            MF_RS.Remotes.JobAction:FireServer("StartJob", "Janitor")
            local t = 0
            repeat
                task.wait(0.5); t = t + 0.5
            until MF_IS_IN_JOB() or t >= 10
            if MF_IS_IN_JOB() then
                NOTIFY("Mop Farm", "✅ Janitor job active!", 3)
            else
                NOTIFY("Mop Farm", "⚠️ Job did not respond, continuing...", 3)
            end
        end
        ...
        local function MF_CLEAN_ALL()
            local janitor = workspace.Jobs.Janitor_Deli
            local cleaned, skipped = 0, 0
            for _, child in ipairs(janitor:GetChildren()) do
                if not _G.EXE.MF_RUNNING then break end
                if child.Name ~= "Dirt" then continue end
                local pp = child:FindFirstChild("ProximityPrompt")
                if not pp or not pp.Enabled then
                    skipped = skipped + 1
                    continue
                end
                MF_DO_TP(child.Position + Vector3.new(0, 3, 0))
                task.wait(0.2)
                MF_CLEAN_DIRT(pp, child)
                cleaned = cleaned + 1
            end
            NOTIFY("Mop Farm", "Round done | Cleaned: " .. cleaned .. " | Skipped: " .. skipped, 3)
        end

        -- Auto-shutoff connections (death + job monitor)
        local MF_CONN_DEATH = nil
        local MF_CONN_JOB   = nil

        _G.MF_STOP          = function()
            _G.EXE.MF_RUNNING = false
            MF_CAM_RESET()
            -- Apagar el toggle visual si existe
            if _G.MF_UI_OFF then pcall(_G.MF_UI_OFF) end
            if _G.EXE.MF_THREAD then
                pcall(task.cancel, _G.EXE.MF_THREAD)
                _G.EXE.MF_THREAD = nil
            end
            -- Desconectar monitores
            if MF_CONN_DEATH then
                pcall(function() MF_CONN_DEATH:Disconnect() end); MF_CONN_DEATH = nil
            end
            if MF_CONN_JOB then
                pcall(function() MF_CONN_JOB:Disconnect() end); MF_CONN_JOB = nil
            end
            NOTIFY("Mop Farm", "Stopped.", 3)
        end

        _G.MF_START         = function()
            if _G.EXE.MF_RUNNING then return end
            _G.EXE.MF_RUNNING = true
            NOTIFY("Mop Farm", "Starting! Teleporting to Deli...", 3)

            -- ── MONITOR 1: Auto-apagado al morir ─────────────────────────────
            local function MF_HOOK_DEATH(char)
                if not char then return end
                local hum = char:FindFirstChildOfClass("Humanoid")
                    or char:WaitForChild("Humanoid", 5)
                if not hum then return end
                if MF_CONN_DEATH then pcall(function() MF_CONN_DEATH:Disconnect() end) end
                MF_CONN_DEATH = hum.Died:Connect(function()
                    if _G.EXE.MF_RUNNING then
                        NOTIFY("Mop Farm", "❌ Player died — auto stop.", 4)
                        _G.MF_STOP()
                    end
                end)
            end
            MF_HOOK_DEATH(LPLR.Character)
            LPLR.CharacterAdded:Connect(function(char)
                if _G.EXE.MF_RUNNING then MF_HOOK_DEATH(char) end
            end)

            -- ── MONITOR 2: Auto-apagado al salir del job ─────────────────────
            pcall(function()
                local jd = LPLR:WaitForChild("Job_Data", 10)
                local cj = jd and jd:WaitForChild("CurrentJob", 10)
                if cj then
                    if MF_CONN_JOB then pcall(function() MF_CONN_JOB:Disconnect() end) end
                    MF_CONN_JOB = cj:GetPropertyChangedSignal("Value"):Connect(function()
                        if _G.EXE.MF_RUNNING and cj.Value ~= "Janitor" then
                            NOTIFY("Mop Farm", "❌ Job left — auto stop.", 4)
                            _G.MF_STOP()
                        end
                    end)
                end
            end)
            _G.EXE.MF_THREAD = task.spawn(function()
                MF_DO_TP(Vector3.new(-410.43, 4.87, 452.29))
                task.wait(0.5)
                MF_START_JOB()
                task.wait(0.5)
                MF_EQUIP_MOP()
                task.wait(0.3)
                NOTIFY("Mop Farm", "Loop running!", 3)
                while _G.EXE.MF_RUNNING do
                    if not MF_IS_IN_JOB() then
                        NOTIFY("Mop Farm", "Job lost, restarting...", 3)
                        MF_DO_TP(Vector3.new(-410.43, 4.87, 452.29))
                        task.wait(0.5)
                        MF_START_JOB()
                        task.wait(0.5)
                    ...
                    local char = LPLR.Character
                    if char and not char:FindFirstChild("Mop") then
                        MF_EQUIP_MOP()
                        task.wait(0.2)
                    end
                    pcall(MF_CLEAN_ALL)
    ...
    do
        ...
        local function MK_JOB_TOG(label, order, onEnable, onDisable)
            ...
            -- Retorna función para apagar el toggle visualmente desde código
            return function()
                if IS_ON then
                    IS_ON = false
                    TWN(PILL, { BackgroundColor3 = CFG.COL.GRY })
        ...
        _G.MF_UI_OFF = MK_JOB_TOG("Mop Farm", 4, _G.MF_START, _G.MF_STOP)
    end

    -- ── MISC TAB — MOVEMENT CARD ──────────────────────────────────
    local MISC_UI = {} -- upvalues storage for MISC sub-sections
    local function SETUP_MOVEMENT()
        ...
        local function startCarFly()
            ...
            if not vehicle then
                NOTIFY("Car Fly", "No hay vehiculo!", 3)
                return false
            end
            local root = CF_GET_ROOT(vehicle)
            if not root then
                NOTIFY("Car Fly", "Sin PrimaryPart!", 3)
                return false
            end
            root.Anchored    = false
        ...
        -- ── UI ───────────────────────────────────────────────────
        MISC_UI.WRAP = Instance.new("Frame", P_MSC)
        MISC_UI.WRAP.Size = UDim2.new(1, -10, 0, 0)
        MISC_UI.WRAP.AutomaticSize = Enum.AutomaticSize.Y
        MISC_UI.WRAP.BackgroundTransparency = 1
        MISC_UI.WRAP.BorderSizePixel = 0
        MISC_UI.WRAP.LayoutOrder = 1

        -- ── LEFT COLUMN CONTAINER ────────────────────────────────
        MISC_UI.L = Instance.new("Frame", MISC_UI.WRAP)
        MISC_UI.L.Size = UDim2.new(0.5, -5, 0, 0)
        MISC_UI.L.Position = UDim2.new(0, 0, 0, 0)
        MISC_UI.L.BackgroundTransparency = 1
        MISC_UI.L.AutomaticSize = Enum.AutomaticSize.Y

        local COL_L_LAY = Instance.new("UIListLayout", MISC_UI.L)
        COL_L_LAY.SortOrder = Enum.SortOrder.LayoutOrder
        COL_L_LAY.Padding = UDim.new(0, 10)

        local MOV_CARD = Instance.new("Frame", MISC_UI.L)
        MOV_CARD.Size = UDim2.new(1, 0, 0, 0)
        ...
        local function MK_MOV_ROW(label, order, onToggle, initVal, minV, maxV, onSlide)
            ...
            if IS_PC then
                ...
                KB_BOX.MouseButton1Click:Connect(function()
                    ...
                    KB_BOX.TextColor3 = CFG.COL.YEL
                    TWN(KB_BOX,
                        { BackgroundColor3 = Color3.fromRGB(50, 45, 20), BackgroundTransparency = 0.5, TextSize = 8 },
                        0.1)
                    ...
                    conn = game:GetService("UserInputService").InputBegan:Connect(function(inp, gpe)
                        ...
                        if inp.KeyCode == Enum.KeyCode.Escape then
                            ...
                            KB_BOX.Text = "—"
                            KB_BOX.TextColor3 = Color3.new(1, 1, 1)
                            TWN(KB_BOX, { BackgroundColor3 = CFG.COL.BG, BackgroundTransparency = 0.82, TextSize = 10 },
                                0.1)
                            return
                        end
    ...
    local function SETUP_CAR_MODS()
        ...
        MISC_UI.R = Instance.new("Frame", MISC_UI.WRAP)
        MISC_UI.R.Size = UDim2.new(0.5, -5, 0, 0)
        MISC_UI.R.Position = UDim2.new(0.5, 5, 0, 0)
        MISC_UI.R.BackgroundTransparency = 1
        MISC_UI.R.AutomaticSize = Enum.AutomaticSize.Y

        local COL_R_LAY = Instance.new("UIListLayout", MISC_UI.R)
        COL_R_LAY.SortOrder = Enum.SortOrder.LayoutOrder
        COL_R_LAY.Padding = UDim.new(0, 10)

        -- ── CAR SPAWNER CARD ────────────────────────────────────
        local CAR_CARD = Instance.new("Frame", MISC_UI.R)
        ...
        if VEH_DATA then
            VEH_DATA.ChildAdded:Connect(function()
                task.wait(0.2)
                CAR_REBUILD_LIST()
            end)
            VEH_DATA.ChildRemoved:Connect(function()
                task.wait(0.2)
                CAR_REBUILD_LIST()
            end)
            for _, v in ipairs(VEH_DATA:GetChildren()) do
        ...
        local TOOLS_LIST = {
            {
                ...
                action = function()
                    ...
                    if not vehicle then
                        NOTIFY("Car Tools", "❌ No car spawned!", 3)
                        return
                    end
    ...
    -- ── OTHERS CARD ───────────────────────────────────────────────
    -- ── OTHERS SECTION ──────────────────────────────────────────
    local function BUILD_OTHERS(MISC_UI, CFG, LPLR_V)
        local OTH_CARD = Instance.new("Frame", MISC_UI.L)
        OTH_CARD.Size, OTH_CARD.AutomaticSize = UDim2.new(1, 0, 0, 0), Enum.AutomaticSize.Y
        OTH_CARD.BackgroundColor3, OTH_CARD.BackgroundTransparency = Color3.fromRGB(22, 22, 30), 0.2
        OTH_CARD.BorderSizePixel, OTH_CARD.LayoutOrder = 0, 2
        RND(OTH_CARD, 12); STR(OTH_CARD, CFG.COL.ACC, 1).Transparency = 0.75

        Instance.new("UIListLayout", OTH_CARD).Padding = UDim.new(0, 8)
        local OTH_PAD = Instance.new("UIPadding", OTH_CARD)
        OTH_PAD.PaddingTop, OTH_PAD.PaddingBottom = UDim.new(0, 12), UDim.new(0, 14)
        OTH_PAD.PaddingLeft, OTH_PAD.PaddingRight = UDim.new(0, 12), UDim.new(0, 12)

        local OTH_HDR = Instance.new("Frame", OTH_CARD)
        OTH_HDR.Size, OTH_HDR.BackgroundTransparency = UDim2.new(1, 0, 0, 28), 1

        local OTH_ICN = Instance.new("ImageLabel", OTH_HDR)
        OTH_ICN.Size, OTH_ICN.Position = UDim2.new(0, 24, 0, 24), UDim2.new(0, 0, 0.5, -12)
        OTH_ICN.BackgroundTransparency, OTH_ICN.Image, OTH_ICN.ImageColor3 = 1, "rbxassetid://6031225819", CFG.COL.ACC

        local OTH_TTL = Instance.new("TextLabel", OTH_HDR)
        OTH_TTL.Size, OTH_TTL.Position, OTH_TTL.BackgroundTransparency = UDim2.new(1, -34, 1, 0), UDim2.new(0, 32, 0, 0),
            1
        OTH_TTL.Text, OTH_TTL.TextColor3, OTH_TTL.Font, OTH_TTL.TextSize = "Others", CFG.COL.TXT, Enum.Font.GothamBold,
            15
        OTH_TTL.TextXAlignment = Enum.TextXAlignment.Left

        local OTH_DIV = Instance.new("Frame", OTH_CARD)
        OTH_DIV.Size, OTH_DIV.BorderSizePixel = UDim2.new(1, 0, 0, 1), 0
        OTH_DIV.BackgroundColor3, OTH_DIV.BackgroundTransparency = CFG.COL.ACC, 0.85

        local function MK_OTH_TOG(label, order, callback)
            local ROW = Instance.new("Frame", OTH_CARD)
            ROW.Size, ROW.BackgroundTransparency, ROW.LayoutOrder = UDim2.new(1, 0, 0, 36), 1, order

            local LBL = Instance.new("TextLabel", ROW)
            LBL.Size, LBL.BackgroundTransparency, LBL.Text = UDim2.new(1, -60, 1, 0), 1, label
            LBL.TextColor3, LBL.Font, LBL.TextSize, LBL.TextXAlignment = CFG.COL.TXT, Enum.Font.Gotham, 14,
                Enum.TextXAlignment.Left

            local PILL = Instance.new("Frame", ROW)
            PILL.Size, PILL.Position, PILL.BackgroundColor3 = UDim2.new(0, 46, 0, 26), UDim2.new(1, -46, 0.5, -13),
                CFG.COL.GRY
            RND(PILL, 13)

            local KNOB = Instance.new("Frame", PILL)
            KNOB.Size, KNOB.Position, KNOB.BackgroundColor3 = UDim2.new(0, 20, 0, 20), UDim2.new(0, 3, 0.5, -10),
                Color3.new(1, 1, 1)
            RND(KNOB, 10)
            ...
            function obj.SET(state)
                if state == IS_ON then return end
                IS_ON = state
                TWN(PILL, { BackgroundColor3 = IS_ON and CFG.COL.GRN or CFG.COL.GRY })
                TWN(KNOB, { Position = IS_ON and UDim2.new(1, -23, 0.5, -10) or UDim2.new(0, 3, 0.5, -10) })
                if callback then callback(IS_ON) end
            end

            local BTN = Instance.new("TextButton", PILL)
            BTN.Size, BTN.BackgroundTransparency, BTN.Text, BTN.ZIndex = UDim2.new(1, 0, 1, 0), 1, "", 5
            BTN.MouseButton1Click:Connect(function() obj.SET(not IS_ON) end)
            return obj
        end

        local DEP_TOG, DEP_CF = nil, nil
        DEP_TOG = MK_OTH_TOG("Deposit All Money", 1, function(on)
            if _G.DEP_TID then
                pcall(function() task.cancel(_G.DEP_TID) end); _G.DEP_TID = nil
            end
            ...
            _G.DEP_TID = task.spawn(function()
                ...
                local char = LPLR_V.Character or LPLR_V.CharacterAdded:Wait()
                local hrp = char:WaitForChild("HumanoidRootPart")
                DEP_CF = hrp.CFrame
                NOTIFY("ATM", "Position saved. Starting...", 2)

                local function BP(pos)
                    local c = LPLR_V.Character or LPLR_V.CharacterAdded:Wait()
                    local h = c:WaitForChild("HumanoidRootPart")
                    RS_L.Remotes.Spawn:FireServer(S_PT)
                    local w = 0
                    repeat
                        task.wait(0.05); w = w + 0.05
                    until (h.Position - S_POS).Magnitude < 50 or w > 3
                    h.Anchored = true; task.wait(0.1)
                    local att = 0
                    repeat
                        h.CFrame = CFrame.new(pos); task.wait(0.1); att = att + 1
                    until (h.Position - pos).Magnitude < 3 or att > 10
                    h.Anchored = false; task.wait(0.2)
                end

                local function G_ATM()
                    local r = LPLR_V.Character and LPLR_V.Character:FindFirstChild("HumanoidRootPart")
                    if not r then return nil end
                    local cl, d_cl = nil, math.huge
                    local map = workspace:FindFirstChild("Map")
                    local atms = map and map:FindFirstChild("ATMs")
                    if not atms then return nil end
                    for _, a in pairs(atms:GetChildren()) do
                        local px = a:FindFirstChild("ProxPart")
                        local pr = px and px:FindFirstChild("ProximityPrompt")
                        if pr and pr.Enabled then
                            local d = (r.Position - px.Position).Magnitude
                            if d < d_cl then d_cl, cl = d, px end
                        end
                    end
                    return cl
                end

                if tonumber(bank.Value) >= 5000000 then
                    NOTIFY("ATM", "Bank at limit ($5M)", 3); DEP_TOG.SET(false); return
                end
                local prox = G_ATM()
                if not prox then
                    NOTIFY("ATM", "No active ATM found", 3); DEP_TOG.SET(false); return
                end

                BP(prox.Position + Vector3.new(0, 0, 3))
                local cam = workspace.CurrentCamera
                cam.CameraType = Enum.CameraType.Scriptable
                ...
                if tonumber(wallet.Value) > 0 then
                    while tonumber(bank.Value) < 5000000 and tonumber(wallet.Value) > 0 do
                        local depVal = math.min(1000000, tonumber(wallet.Value), 5000000 - tonumber(bank.Value))
                        local success = pcall(function() ATMAction:FireServer("Deposit", depVal) end)
                        if not success then break end
                        task.wait(1.5)
                    end
                end

                local cc = LPLR_V.Character; if cc and cc:FindFirstChild("HumanoidRootPart") then cc.HumanoidRootPart.Anchored = false end
                local pg = LPLR_V:WaitForChild("PlayerGui")
                for _, o in pairs(pg:GetDescendants()) do if o.Name:lower():find("atm") then o:Destroy() end end
                cam.CameraType = Enum.CameraType.Custom; BP(DEP_CF.Position); NOTIFY("ATM", "Deposit completed!", 3)
                DEP_TOG.SET(false)
            end)
        end)

        local BYPASS_CONNS = {}
        local function FORCE_SIT_L(s, r, h)
            if not s or not r or not h then return end
            pcall(function() s.Disabled = false end)
            if firetouchinterest then
                pcall(function()
                    firetouchinterest(r, s, 0); task.wait(0.05); firetouchinterest(r, s, 1)
                end); task.wait(0.1)
            end
            if h.SeatPart ~= s then
                pcall(function()
                    r.CFrame = s.CFrame * CFrame.new(0, 2, 0); task.wait(0.05); h.Sit = true; s:Sit(h)
                end); task.wait(0.1)
            end
            if h.SeatPart ~= s then
                pcall(function()
                    ...
                    w.Name = "TempSeatWeld"
                    w.Part0 = s; w.Part1 = r; w.C0 = CFrame.new(0, 0.5, 0); w.Parent = s
                    h.Sit = true; s:Sit(h)
                    task.delay(1, function() if w and w.Parent then w:Destroy() end end)
                end)
            end
        end

        MK_OTH_TOG("Bypass Locked Cars", 3, function(on)
            _G.EXE.BYPASS_CARS_ON = on
            if not on then
                for _, c in pairs(BYPASS_CONNS) do if c then c:Disconnect() end end; table.clear(BYPASS_CONNS); return
            end

            local function findSeat()
                local char = LPLR_V.Character
                local hrp = char and char:FindFirstChild("HumanoidRootPart")
                if not hrp then return nil end
                local v_f = workspace:FindFirstChild("Vehicles")
                if not v_f then return nil end
                local closest, dist = nil, 20
                for _, v in ipairs(v_f:GetChildren()) do
            ...
            local function handleBypass(target)
                local t = target or findSeat()
                if not t then return end
                local char = LPLR_V.Character
                local hrp = char and char:FindFirstChild("HumanoidRootPart")
                local hum = char and char:FindFirstChildOfClass("Humanoid")
                if hrp and hum then FORCE_SIT_L(t, hrp, hum) end
            end

            local function setupHum(char)
                local hum = char:WaitForChild("Humanoid", 10)
                if hum then
                    BYPASS_CONNS.SEATED = hum.Seated:Connect(function(isS, seat)
                        if isS and seat and _G.EXE.BYPASS_CARS_ON then handleBypass(seat) end
                    end)
                end
            end

            if LPLR_V.Character then setupHum(LPLR_V.Character) end
            BYPASS_CONNS.CHAR = LPLR_V.CharacterAdded:Connect(setupHum)

            BYPASS_CONNS.INPUT = UIS.InputBegan:Connect(function(i, g)
                if not g and _G.EXE.BYPASS_CARS_ON and i.KeyCode == Enum.KeyCode.E then handleBypass() end
            end)

            local PPS = game:GetService("ProximityPromptService")
            local function onPrompt(prompt, player)
                if player == LPLR_V and _G.EXE.BYPASS_CARS_ON then
                    local p = prompt.Parent
                    handleBypass((p:IsA("Seat") or p:IsA("VehicleSeat")) and p or nil)
                end
            end
            BYPASS_CONNS.P_BEGIN = PPS.PromptButtonHoldBegan:Connect(onPrompt)
            BYPASS_CONNS.P_TRIG = PPS.PromptTriggered:Connect(onPrompt)
        end)

        local DIRTY_TOG, DIRTY_ACTIVE = nil, false
        DIRTY_TOG = MK_OTH_TOG("Clear All Dirty Money", 4, function(on)
            DIRTY_ACTIVE = on
            if CLEAR_THREAD then
                pcall(function() task.cancel(CLEAR_THREAD) end); CLEAR_THREAD = nil
            end
            if not on then return end
            ...
            CLEAR_THREAD = task.spawn(function()
                ...
                local function LAUNDRY_TP(pos)
                    ...
                    repeat
                        task.wait(0.05); w = w + 0.05
                        char = LPLR_V.Character or LPLR_V.CharacterAdded:Wait()
                        hrp = char:WaitForChild("HumanoidRootPart")
                    until (hrp.Position - S_POS).Magnitude < 50 or w > 3
                    hrp.CFrame = CFrame.new(pos)
                end

                local function supportsFirePrompt()
                    if fireproximityprompt then
                        local d = Instance.new("ProximityPrompt")
                        local ok = pcall(function() fireproximityprompt(d) end)
                        d:Destroy(); return ok
                    end
                    return false
                end
                local useFireP = supportsFirePrompt()

                local function getDirtyMoney()
                    return LPLR_V.Backpack:FindFirstChild("Dirty Money") or
                        (LPLR_V.Character and LPLR_V.Character:FindFirstChild("Dirty Money"))
                end

                local function equipDirtyMoney()
                    local t = LPLR_V.Backpack:FindFirstChild("Dirty Money")
                    local c = LPLR_V.Character; if not c then return false end
                    local h = c:FindFirstChildOfClass("Humanoid")
                    if t and h then
                        h:EquipTool(t); task.wait(0.3); return true
                    end
                    return c:FindFirstChild("Dirty Money") ~= nil
                end

                local function getDMVal()
                    return (LPLR_V:FindFirstChild("Player_Data") and LPLR_V.Player_Data:FindFirstChild("DirtyMoney") and LPLR_V.Player_Data.DirtyMoney.Value) or
                        0
                end

                local function getWallet()
                    return (LPLR_V:FindFirstChild("Player_Data") and LPLR_V.Player_Data:FindFirstChild("Wallet") and LPLR_V.Player_Data.Wallet.Value) or
                        0
                end
                ...
                local initP = nil
                pcall(function() initP = LPLR_V.Character.HumanoidRootPart.Position end)

                if not getDirtyMoney() then
                    NOTIFY("Laundry", "❌ No Dirty Money found!", 3); DIRTY_TOG.SET(false); return
                end

                NOTIFY("Laundry", "Starting... Teleporting to Laundromat", 2)
                LAUNDRY_TP(Vector3.new(-53.99, 4.30, -179.17))
                task.wait(0.5)

                local first = true
                while getDMVal() > 0 and DIRTY_ACTIVE do
                    if not equipDirtyMoney() then break end
                    task.wait(0.2)

                    local wAntes = getWallet()
                    local laundromat = workspace.Map.Interactables.Laundromat
                    local pp = laundromat:GetChildren()[7]:FindFirstChild("ProximityPrompt")
                    if not pp then break end

                    NOTIFY("Laundry", "Washing... DM: $" .. getDMVal(), 2)
                    useP(pp)

                    local elapsed, paid = 0, false
                    while elapsed < 38 and DIRTY_ACTIVE do
                        task.wait(1); elapsed = elapsed + 1
                        if getWallet() > wAntes then
                            NOTIFY("Laundry", "✅ Payment received! +$" .. (getWallet() - wAntes), 3)
                            paid = true; break
                        end
                    end
                    if not paid then NOTIFY("Laundry", "⚠️ Cooldown finished or timeout", 2) end
                    task.wait(0.5)
                end

                if DIRTY_ACTIVE then
                    NOTIFY("Laundry", "Finished! All money washed.", 3)
                    if initP then LAUNDRY_TP(initP) end
                end
                DIRTY_TOG.SET(false)
            end)
        end)
    end
    BUILD_OTHERS(MISC_UI, CFG, LPLR)

    local function SETUP_CAR_DEALER()
        print("[NYH] Building Car Dealer Card...")
        -- ── CAR DEALER CARD ────────────────────────────────────
        local DEALER_CARD = Instance.new("Frame", MISC_UI.R)
        ...
        local function FMT_P(v)
            if v >= 1000000 then
                return string.format("%.1fM", v / 1000000):gsub("%.0M", "M")
            elseif v >= 1000 then
        ...
        BUY_BTN.MouseButton1Click:Connect(function()
            if not SEL_CAR then
                NOTIFY("Dealer", "Select a car first!", 3)
                return
            end
            local bank = LPLR.Player_Data.Bank.Value
            if bank < SEL_PRC then
                NOTIFY("Dealer", "Need $" .. SEL_PRC .. " in Bank", 4)
                return
            end
            ...
            if not remote then
                NOTIFY("Dealer", "Remote not found!", 3)
                return
            end
    ...
    local function SETUP_VISUALS()
        ...
        ADD_TGL_KB(C3, "Enable", false, nil, function(v, k)
            ESP_CFG.SilentAim.Enabled = v; ESP_CFG.SilentAim.Keybind = k
        end)
        ADD_ESP_ROW(C3, "Snapline", ESP_CFG.Snapline.Enabled, function(v) ESP_CFG.Snapline.Enabled = v end,
            { { VAL = ESP_CFG.Snapline.Color, CB = function(c) ESP_CFG.Snapline.Color = c end } })
        ADD_SLD(C3, "Snapline Thickness", 1, 5, ESP_CFG.Snapline.Thickness,
            function(v) ESP_CFG.Snapline.Thickness = v end)
    ...
    local function SETUP_CONFIG()
        ...
        local currentName = "Classic" -- default display
        for _, m in ipairs(METHODS) do if m.id == _G.TP_METHOD then currentName = m.name end end
